...
namespace Algorithms.Graph
{
    ...
    public class BreadthFirstSearch<T> : IGraphSearch<T> where T : IComparable<T>
    {
        ...
        {
            BreadthFirstSearch<T>.Bfs(graph, startVertex, action, new HashSet<Vertex<T>>());
        }
        ...
        private static void Bfs(IDirectedWeightedGraph<T> graph, Vertex<T> startVertex, Action<Vertex<T>>? action, HashSet<Vertex<T>> visited)
        {
